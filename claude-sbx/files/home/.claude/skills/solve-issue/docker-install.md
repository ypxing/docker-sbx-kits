...
(service `serverless`, `CONTAINER_SRC=/opt/app`, `SLUG=myproject`):
...
docker compose \
  ...
  run --rm <service> sh -c "<install for each package.json dir>"
```
...
```bash
docker compose ... run --rm --entrypoint sh <service> -c "<install-command>"
```
